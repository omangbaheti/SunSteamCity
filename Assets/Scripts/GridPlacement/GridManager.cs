...
public class GridManager : GridProperties
{
    ...
    private SimpleBuilding currentSimpleBuilding;
    ...
    {
        if(currentSimpleBuilding == null) return;
            
        if ((ulong) currentSimpleBuilding.Price >= ecoManager.SteamCoin) return;
        ecoManager.BuyBuilding(currentSimpleBuilding.Price);
        ...
        GameObject placedBuilding = Instantiate(currentSimpleBuilding.Model, GetWorldPosition(gridPosition), Quaternion.identity);
       
        // ecoManager.AddToIncrementValue(currentBuilding.GetLevelDetails(1).ProduceValue);
        ecoManager.AddToIncrementValue(currentSimpleBuilding.ProductionRate);
        
        List<Vector2Int> occupiedSpaces = currentSimpleBuilding.FillGridPositions(gridPosition,simpleBuilding:currentSimpleBuilding);
        foreach (Vector2Int cell in occupiedSpaces)
        {
            ...
            GridArray[cell.x,cell.y].SetBuilding(placedBuilding.transform, currentSimpleBuilding.ProductionRate);
        } 
    }

    private void UpdateBuilding(SourceBuildingAsset building)
    {
    ...
    private void DestroyBuilding()
    {
        ...
        foreach (Vector2Int cell in occupiedSpaces)
        {
            GridArray[cell.x, cell.y].CellType = SimpleBuildingType.Empty;
